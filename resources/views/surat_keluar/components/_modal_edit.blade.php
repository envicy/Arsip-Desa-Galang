<div class="modal fade" id="modalEdit{{ $item->id_surat_keluar }}" tabindex="-1" aria-labelledby="modalEditLabel{{ $item->id_surat_keluar }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form action="{{ route('surat-keluar.update', $item->id_surat_keluar) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-warning text-dark">
                    <h5 class="modal-title fw-bold" id="modalEditLabel{{ $item->id_surat_keluar }}">
                        <i class="fas fa-edit me-2"></i>Edit Surat Keluar
                    </h5>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" name="tujuan_surat" 
                                   ...
                                   value="{{ old('tujuan_surat', $item->tujuan_surat) }}" required>
                            @error('tujuan_surat')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
